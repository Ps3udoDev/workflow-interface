    ...
    ({ children, position = 'top-left', type = 'grow', direction = 'up', ...others }, ref) => {
        ...
        return (
            <Box ref={ref}>
                ...
                {type === 'collapse' && <Collapse {...others} sx={positionSX}>{children}</Collapse>}
                {type === 'fade' && <Fade {...others} timeout={{ appear: 500, enter: 600, exit: 400 }}><Box sx={positionSX}>{children}</Box></Fade>}
                {type === 'slide' && <Slide {...others} timeout={{ appear: 0, enter: 400, exit: 200 }} direction={direction}><Box sx={positionSX}>{children}</Box></Slide>}
